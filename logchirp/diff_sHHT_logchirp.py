...
ff=h5py.File("logchirp_disp2.hdf5","w")    # output the result for a plot
...
#input_noise_level=0.9
input_noise_level=0.05                     # input noise level to perform sHHT


nfreq=1000                                 # frequency grids
fmin=0.01                                  # frequency range: 0.01 - 5 Hz
fmax=5.0
...
plt.show()
#plt.savefig('diff_HHT_logchirp2.eps',format="eps", dpi=1200)
#plt.close(fig)
...
plt.ylabel('Frequency', fontsize=30)
plt.yticks(fontsize=20)
#plt.savefig('disp_logchirp2.eps',format="eps", dpi=1200)
#plt.close(fig)
plt.show()
