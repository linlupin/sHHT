...
nmdfreq_disp = f['/info/RFD_sHHT'][()]    # Frequency dispersion between IF obtained from sHHT (IF2) and real signal frequency
nmfreq_disp = f['/info/RdFD_sHHT'][()]    # Frequency dispersion ratio between IF2 and real signal frequency
...
#fig=plt.figure(figsize=(20,10))
#y, x = np.meshgrid(input_noise_level, amp)
#Z = ndimage.gaussian_filter(nmdfreq_disp, sigma=1.0, order=0)
...
#plt.colorbar(CS)
#plt.savefig('nmddisp_contour.eps',format="eps", dpi=1200)
#plt.close(fig)
