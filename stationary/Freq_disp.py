...
imf_opts = {'max_iters': 100, 'stop_method':'sd', 'sd_thresh': 0.005, 'energy_thresh': 50}
max = 0
...
#####
        ...
        for ii in np.arange(len(t)-1):
             ...
             ndfreq_disp[ii]=SSF[ii]- freq
             freq_disp[ii]=abs(SSF[ii]-OSF[ii])/OSF[ii]
             nfreq_disp[ii]=abs(SSF[ii]-freq)/freq
       ...
       # We ignore the results obtained in the boundaries (< 1 s and > 9 s)
        ...
        if ind==0 and mfreq_disp[i,w] > 1.0:
              ...
              plt.plot(t[10:990],freqq[10:990],color='black')
